...
)
{
    ...
    private readonly ICommandHandler<CreateCompany.Command, Guid> _createCompanyHandler =
    ...
    /// </summary>
    public CrmCompanyProfile EnrollCompany(
        Guid companyId,
        CompanyStage stage,
        string? notes,
    ...
    )
    {
        var translations = LanguageCodeMapper
            .SupportedLanguageCodes.Select(lc => new CompanyTranslationDto(lc, name, string.Empty))
            .ToList();

        var result = await _createCompanyHandler.Handle(
            new CreateCompany.Command(website, region, translations),
            ct
        );
    ...
    public async Task<string> GetCompanyNameAsync(Guid companyId, CancellationToken ct = default)
    {
        ...
        if (company is null)
            return "Unknown";
        var enTrans = company.Translations.FirstOrDefault(t =>
            t.Language == Kulku.Domain.LanguageCode.English
        );
        return enTrans?.Name
            ?? (company.Translations.Count > 0 ? company.Translations[0].Name : null)
            ?? "Unknown";
    }
    ...
    public async Task EnsureSeededAsync(CancellationToken ct = default)
    {
        ...
        {
            (
                CompanyStage.Relationship,
                "Long-term platform modernization potential.",
                new[] { fintech.Id }
            ),
            (
                CompanyStage.Discovery,
                "Data pipelines + cloud cost optimization; likely recurring needs.",
                new[] { b2b.Id }
            ),
            (
                CompanyStage.Proposal,
                "Product studio; could become a repeat delivery partner.",
                new[] { health.Id }
            ),
            (
                CompanyStage.Parked,
                "Procurement cycles; long horizon. Keep warm without spamming.",
                new[] { publicSector.Id }
            ),
        };
        ...
        {
            var d = contactData[i];
            var c = _store.AddContact(
                enrolledCompanyIds[i],
                NullIfEmpty(d[0]),
                NullIfEmpty(d[1]),
                NullIfEmpty(d[2]),
                NullIfEmpty(d[3]),
        ...
        {
            _store.AddInteraction(
                new InteractionLite(
                    Guid.NewGuid(),
                    enrolledCompanyIds[0],
                    DateTime.Today.AddDays(-10),
                    InteractionDirection.Inbound,
                    InteractionChannel.CvContactForm,
                    true,
                    "Mikko (ex-colleague)",
                    "Colleague",
                    contactIds[0],
                    "Inbound via CV contact form; wants modernization workshop; timeline Q2\u2013Q3.",
                    "Send 1-pager + propose 60min workshop slot",
        ...
        if (enrolledCompanyIds.Count >= 2 && contactIds.Count >= 2)
        {
            _store.AddInteraction(
                new InteractionLite(
                    ...
                    "Reply with 2 relevant references + suggest intro call",
        ...
        if (enrolledCompanyIds.Count >= 3 && contactIds.Count >= 3)
        {
            _store.AddInteraction(
                new InteractionLite(
                    ...
                    "Schedule discovery call + ask for constraints & priorities",
    ...
    private async Task<IReadOnlyList<CompanyTranslationsModel>> GetDbCompaniesAsync(
    ...
    private static CrmCompanyViewModel ToViewModel(
        CompanyTranslationsModel db,
        CrmCompanyProfile? profile
    )
    {
        // Pick English name, fall back to first available
        var enTrans = db.Translations.FirstOrDefault(t =>
            t.Language == Kulku.Domain.LanguageCode.English
        );
        ...
        return new CrmCompanyViewModel(
            db.CompanyId,
            name,
            description,
            db.Website,
